...
class Add(nn.Module):
    """Element-wise sum of multiple input tensors."""
...
_DYSAMPLE_IMPORT_ERROR = None
try:
    from ultralytics.nn.extra_modules import DySample

    DYSAMPLE_AVAILABLE = True
except ImportError as _err:
    _DYSAMPLE_IMPORT_ERROR = _err
    DYSAMPLE_AVAILABLE = False
    _dysample_err_msg = str(_err)
    LOGGER.warning(f"DySample import failed: {_dysample_err_msg}")

    def _missing_dysample(*args, **kwargs):
        raise RuntimeError(f"DySample unavailable: {_dysample_err_msg}")

    globals()["DySample"] = _missing_dysample
...
def parse_model(d, ch, verbose=True, dataset_config=None):
    ...
    for i, layer_config in enumerate(d["backbone"] + d["head"]):  # from, number, module, args, [input_type, hook]
        ...
        if m in base_modules:
            ...
            if isinstance(f, int) or len(f) < 2:
                raise ValueError(f"{m.__name__} expects at least 2 inputs, got {f} at layer {i}")
            c2 = ch[f[0]]
            if any(ch[x] != c2 for x in f):
                raise ValueError(f"{m.__name__} expects equal channels, got {[ch[x] for x in f]} at layer {i}")
        ...
        # 其他融合模块（与多模态路由无关的纯特征融合），要求两路输入空间尺寸一致
            ...
            if len(args) == 0:
                ...
                args[0] = c_left
            c2 = c_left
        elif m in {DepthReliabilityConcatGate, DepthReliabilityConcatGateV2, DepthConcatGateNoDiff}:
            if isinstance(f, int) or len(f) != 2:
                raise ValueError(f"{m.__name__} expects 2 inputs, got {f} at layer {i}")
            c_left, c_right = ch[f[0]], ch[f[1]]
            if c_left != c_right:
                raise ValueError(f"{m.__name__} expects equal channels, got {c_left} and {c_right} at layer {i}")
            if len(args) == 0:
                args.insert(0, c_left)
            elif args[0] is None:
                args[0] = c_left
        ...
        # === C3k2 变体（需要参数重排） ===
            ...
            # DySample: dynamic upsampling, channels stay unchanged
            c1, c2 = ch[f], ch[f]
            # args from YAML remain unchanged, e.g. [2, 'lp']
        elif m is GSConvE:
