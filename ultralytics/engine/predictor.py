    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                # 注意：多模态场景下，预处理可能将两路输入合成为单样本（如 RGB+X -> 6 通道），
                # 此时 len(im0s) 可能大于实际预测结果数。按结果长度迭代可避免越界与漏写。
                n = len(self.results)
                try:
                    for i, res in enumerate(self.results):
                        ...
                        denom = max(n, 1)
                        res.speed = {
                            "preprocess": profilers[0].dt * 1e3 / denom,
                            "inference": profilers[1].dt * 1e3 / denom,
                            "postprocess": profilers[2].dt * 1e3 / denom,
                        }
                        if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                            path_i = paths[i] if i < len(paths) else paths[0]
                            s_i = s[i] if i < len(s) else ""
                            s_i += self.write_results(i, Path(path_i), im, s)
                            if i < len(s):
                                s[i] = s_i
                            else:
                                s.append(s_i)
